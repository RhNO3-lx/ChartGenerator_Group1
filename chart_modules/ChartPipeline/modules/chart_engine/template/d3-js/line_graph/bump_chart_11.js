...
function makeChart(containerSelector, data) {
    ...
    groups.forEach(group => {
        ...
        groupData.forEach(d => {
            ...
            // 计算六边形颜色，实现从左到右由暗变亮的效果
            let finalHexColor = colorScale(group); // 默认为分组的基础颜色

            if (timePoints.length > 1) {
                const baseColorObject = d3.color(colorScale(group));
                if (baseColorObject) { // 确保颜色对象有效
                    // 定义亮度的最大调整参数（例如，0.6 意味着最暗的点比基色暗约20%，最亮的点亮约25%）
                    const brightnessParamMax = 2;

                    // 计算亮度调整值 kFactor:
                    // timePointIndex = 0 (最左/最早) => kFactor = -brightnessParamMax (最暗)
                    // timePointIndex = numTimePoints - 1 (最右/最晚) => kFactor = brightnessParamMax (最亮)
                    ...
                    if (kFactor > 0) {
                        finalHexColor = baseColorObject.brighter(kFactor).toString();
                    } else if (kFactor < 0) {
                        finalHexColor = baseColorObject.darker(Math.abs(kFactor)).toString();
                    } else {
                        // kFactor is 0, use base color (already set in finalHexColor)
                        finalHexColor = baseColorObject.toString(); //确保是字符串格式
                    }
                }
            }
            ...
            // 如果六边形足够大，可以放置标签，则将标签放在六边形内部
            // 六边形内接圆半径为外接圆半径的0.866(sqrt(3)/2)，所以要乘以这个因子
            const innerCircleRadius = circleRadius * 0.866;

            if (innerCircleRadius * 2 >= textMetrics.width + 6) { // 加上一些内边距
                g.append("text")
                    .attr("x", cx)
                    .attr("y", cy)
                ...
                g.append("text")
                    ...
                    .style("font-size", annotationFontSize + "px")
                    .style("font-weight", typography.annotation.font_weight)
                    .style("fill", "#333333") // 统一使用黑色作为六边形外部标签颜色
                    .text(formattedValue);
            }
        });
    });
