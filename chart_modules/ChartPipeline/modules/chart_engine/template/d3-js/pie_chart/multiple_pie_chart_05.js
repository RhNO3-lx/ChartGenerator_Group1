...
function makeChart(containerSelector, data) {
    ...
    // 设置视觉效果变量的默认值
    variables.has_shadow = 0
    variables.has_stroke = 1
    ...
    // 计算图表半径 - 调整饼图尺寸
    const radius = Math.min(innerCellWidth / 2, innerCellHeight / 2.2) * 1.1; // 饼图适中尺寸
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < itemsInThisRow; col++) {
            ...
            // 调整饼图Y位置，确保有足够空间
            const chartCenterY = margin.top + (row + 0.45) * cellHeight;
            ...
            const labelArc = d3.arc()
                .innerRadius(radius * 0.55) // 调整为适中值
                .outerRadius(radius * 1.05); // 调整为适中值
            ...
            labels.each(function(d) {
                ...
                // 获取当前扇区的颜色
                const sectorColor = colors.field[d.data[groupField]] || colors.other.primary;
                // 根据扇区颜色决定文本颜色
                ...
                if (percent > 0.07) {
                    ...
                    // 设置标签位置 - 从centroid向外移动一定距离，适当调整
                    const x = centroid[0] + unitX * (radius * 0.22); // 适中位移值
                    const y = centroid[1] + unitY * (radius * 0.22);
                    ...
                    // 设置标签位置 - 控制向外移动的距离，防止超出边界
                    const x = centroid[0] + unitX * (radius * 0.6); // 增加距离，减少重叠
                    const y = centroid[1] + unitY * (radius * 0.6);
            ...
            chartGroup.append("text")
                .attr("x", 0)
                .attr("y", -radius - 10) // 减小与饼图的距离，从18改为10
                .attr("text-anchor", "middle")
                .style("font-family", typography.label.font_family)
