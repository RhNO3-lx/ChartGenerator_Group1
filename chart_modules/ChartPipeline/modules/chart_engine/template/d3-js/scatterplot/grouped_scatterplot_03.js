...
function makeChart(containerSelector, data) {
    ...
    function findOptimalPosition(d, allPoints, currentPositions = {}) {
        ...
        for (const pos of positions) {
            ...
            for (const p of allPoints) {
                ...
                // 检查与点的重叠
                const pointRadius = circleRadius; // Use the calculated circleRadius
                ...
                if (pPos) {
                    ...
                    if (pPos.anchor === "start") {
                        ...
                        otherY1 = pY + pPos.y; // Align based on text-anchor: middle baseline
                    } else { // end
                        otherX1 = pX + pPos.x - otherBBox.width;
                        otherY1 = pY + pPos.y - otherBBox.height/2;
                    }
                     const otherX2 = otherX1 + otherBBox.width;
                     const otherY2 = otherY1 + otherBBox.height;

                    if (labelX1 < otherX2 && labelX2 > otherX1 &&
                        labelY1 < otherY2 && labelY2 > otherY1) {
                        hasOverlap = true;
                        break;
                    }
                }
    ...
    // Add circles for data points (original style)
    ...
    points.append("text")
        ...
        .style("opacity", d => labelPositions[d[xField]].canShow ? 1 : 0) // Control visibility based on canShow flag
        .text(d => d[xField]);
