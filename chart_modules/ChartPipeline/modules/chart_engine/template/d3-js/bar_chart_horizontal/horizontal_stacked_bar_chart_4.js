...
function makeChart(containerSelector, data) {
    ...
    // 设置边距，确保左侧有足够空间放置维度标签
    // 添加一些额外空间作为标签和条形图之间的间距
    const labelMargin = 5;
    const margin = {
        top: 90,       // 顶部留出标题空间
        right: 5,     // 右侧边距
        bottom: 40,    // 底部边距
        left: maxLabelWidth + labelMargin // 左侧边距，为维度标签预留空间
    };
    ...
    const maxRadius = Math.min(yScale.bandwidth() * 1.6,circleChartWidth*0.8)  // 最大半径可以是两个条形的高度
    
    // 替换圆形比例尺为正方形边长比例尺
    // 注意：在d3.scaleSqrt中，我们输入的值按平方根缩放，输出将是正方形的边长
    // 这样可以确保最终生成的正方形面积与数值大小成正比
    ...
    sortedDimensions.forEach((dimension, index) => {
        ...
        // 1. 添加维度标签
        g.append("text")
            .attr("x", -labelMargin) // 放在left margin减去间距的位置
            .attr("y", centerY)
            .attr("dy", "0.35em")
            .attr("text-anchor", "end") // 右对齐
            .style("font-family", typography.label.font_family)
        ...
        if (circleValue > 0) {
            const squareSide = squareSideScale(circleValue);  // 获取正方形边长
            const squareX = barChartWidth + circleChartWidth / 2 - squareSide / 2;  // 水平居中
            
            // 绘制正方形
            g.append("rect")
                .attr("x", squareX)
                .attr("y", centerY - squareSide / 2)  // 垂直居中
            ...
            // 计算正方形中心坐标
            const squareCenterX = squareX + squareSide / 2;
            
            // 如果正方形足够大，将文本放在正方形内；否则放在上方
            if (squareSide > textWidth + 10) {
                g.append("text")
                    .attr("x", squareCenterX)
                    .attr("y", centerY)
                    .attr("dy", "0.35em")
