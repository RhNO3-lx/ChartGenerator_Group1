...
REQUIREMENTS_BEGIN
{
    ...
    "chart_name": "horizontal_stacked_bar_chart_8_hand",
...
function makeChart(containerSelector, data) {
    ...
    // 添加组标签（在条形图上半部分）
    layers.selectAll(".group-label-bg")
        .data(d => d)
        .enter().append("rect")
        .attr("class", "group-label-bg")
        .attr("y", d => yScale(d.data.period) + yScale.bandwidth() * 0.35 - 8) // 位置稍微调整以包围文本
        .attr("x", d => xScale(d[0]) + 5) // 与文本对齐
        .attr("height", 16) // 固定高度包围文本
        .attr("width", 0) // 宽度将在文本添加后调整
        .style("fill", "#ffffff")
        .style("opacity", 0.5);
    ...
    layers.selectAll(".group-label")
        ...
        .each(function(d, i) {
            ...
            if (textContent) {
                const fontSize = text.style("font-size");
                const textWidth = getTextWidth(textContent, fontSize);
                // 获取对应的背景矩形并设置宽度
                const bgRect = d3.select(this.parentNode).selectAll(".group-label-bg").filter((_, j) => j === i);
                bgRect.attr("width", textWidth + 10); // 加10px的内边距
            }
        });

    // 添加数值标注背景（在条形图下半部分）
    layers.selectAll(".value-label-bg")
        .data(d => d)
        .enter().append("rect")
        .attr("class", "value-label-bg")
        .attr("y", d => yScale(d.data.period) + yScale.bandwidth() * 0.7 - 8) // 位置调整以包围文本
        .attr("x", d => xScale(d[0]) + 5) // 与文本对齐
        .attr("height", 16) // 固定高度包围文本
        .attr("width", 0) // 宽度将在文本添加后调整
        .style("fill", "#ffffff")
    ...
    layers.selectAll(".value-label")
        ...
        .each(function(d, i) {
            ...
            if (textContent) {
                ...
                const textWidth = getTextWidth(textContent, fontSize);
                // 获取对应的背景矩形并设置宽度
                const bgRect = d3.select(this.parentNode).selectAll(".value-label-bg").filter((_, j) => j === i);
                bgRect.attr("width", textWidth + 10); // 加10px的内边距
            }
        });
