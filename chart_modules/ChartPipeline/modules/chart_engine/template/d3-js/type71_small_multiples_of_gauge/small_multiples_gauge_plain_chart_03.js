...
function makeChart(containerSelector, data) {
    ...
    processedData.forEach((d, index) => {
        ...
        const centerY = subChartHeight / 2;
        const maxRadius = Math.min((subChartWidth - subMargin * 2) * 0.85, (subChartHeight - subMargin * 2) * 0.85) / 2;
        const arcThickness = Math.max(12, maxRadius * 0.25);
        
        // 添加x标签，在圆弧正下方
        const labelY = centerY + maxRadius + 25; // 圆弧正下方
        ...
        // 数值标签放在12点钟正左边，右对齐
        const valueLabelX = centerX - 15; // 12点钟正左边15px
        const valueLabelY = centerY - maxRadius + arcThickness * 0.5; // 12点钟正左边（与圆弧最高点同高）
        
        // 字体大小小于圆弧宽度
        const fontSize = Math.min(16, arcThickness * 0.8); // 确保字体小于圆弧宽度
