...
REQUIREMENTS_BEGIN
{
    ...
    "chart_name": "grouped_scatterplot_plain_chart_03",
...
function makeChart(containerSelector, data) {
    ...
    const findOptimalPosition = (d, allPoints, currentPositions = {}) => {
        ...
        for (const pos of positions) {
            ...
            for (const p of allPoints) {
                ...
                if (distance < triangleSize + Math.sqrt(labelWidth * labelWidth + labelHeight * labelHeight) / 2) {
    ...
    // 确定三角形大小 - 智能尺寸计算（扩大√2倍以实现面积2倍）
    const numPoints = chartData.length;
    let triangleSize = numPoints <= 10 ? 18 - numPoints * 0.7 : 12 - (numPoints - 10) * 0.4;
    triangleSize = Math.max(3, Math.min(18, triangleSize)) * 1.414; // 扩大√2倍
    ...
    // 添加数据点三角形
    points.append("path").attr("class", "mark")
    ...
    groups.forEach(name => {
        ...
        const itemW = triangleSize + 4 + textW;
    ...
    lines.forEach((line, lineIdx) => {
        ...
        line.forEach(item => {
            ...
            itemG.append("path").attr("class", "mark")
                .attr("d", createTrianglePath(triangleSize))
                .attr("transform", `translate(${triangleSize/2}, 0)`)
                .attr("fill", colors.field[item.name] || colors.other.primary).style("opacity", 0.75);
            itemG.append("text").attr("class", "label").attr("x", triangleSize + 4).attr("y", 0)
