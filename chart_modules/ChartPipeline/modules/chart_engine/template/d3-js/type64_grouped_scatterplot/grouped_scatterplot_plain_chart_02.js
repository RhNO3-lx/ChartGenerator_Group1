...
function makeChart(containerSelector, data) {
    ...
    const findOptimalPosition = (d, allPoints, currentPositions = {}) => {
        ...
        for (const pos of positions) {
            ...
            for (const p of allPoints) {
                ...
                if (distance < squareSize + Math.sqrt(labelWidth * labelWidth + labelHeight * labelHeight) / 2) {
    ...
    // 确定正方形大小 - 智能尺寸计算（扩大√2倍以实现面积2倍）
    const numPoints = chartData.length;
    let squareSize = numPoints <= 10 ? 18 - numPoints * 0.7 : 12 - (numPoints - 10) * 0.4;
    squareSize = Math.max(3, Math.min(18, squareSize)) * 1.414; // 扩大√2倍
    ...
    // 添加数据点正方形
    points.append("rect").attr("class", "mark")
        .attr("x", -squareSize/2).attr("y", -squareSize/2)
    ...
    groups.forEach(name => {
        ...
        const itemW = squareSize + 4 + textW;
    ...
    lines.forEach((line, lineIdx) => {
        ...
        line.forEach(item => {
            ...
            itemG.append("rect").attr("class", "mark")
                .attr("x", 0).attr("y", -squareSize/2)
                .attr("width", squareSize).attr("height", squareSize)
                .attr("fill", colors.field[item.name] || colors.other.primary).style("opacity", 0.75);
            itemG.append("text").attr("class", "label").attr("x", squareSize + 4).attr("y", 0)
