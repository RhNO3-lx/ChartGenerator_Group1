...
function makeChart(containerSelector, data) {
    ...
    const leftGroupColor = colors.field[leftBarGroup] || "#4269d0"; // Default left color
    const rightGroupColor = colors.field[rightBarGroup] || "#ff725c"; // Default right color
    ...
    gradientLeft.append("stop")
        .attr("offset", "0%")
        .attr("stop-color", d3.rgb(leftGroupColor).darker(0.3));

    gradientLeft.append("stop")
        .attr("offset", "50%")
        .attr("stop-color", d3.rgb(leftGroupColor).brighter(2.5));
    ...
    gradientRight.append("stop")
        .attr("offset", "0%")
        .attr("stop-color", d3.rgb(rightGroupColor).darker(0.3));

    gradientRight.append("stop")
        .attr("offset", "50%")
        .attr("stop-color", d3.rgb(rightGroupColor).brighter(2.5));
    ...
    const formatValue = (value) => {
        if (value >= 1000000000) {
            return d3.format("~g")(value / 1000000000) + "B" + yUnit;
        } else if (value >= 1000000) {
            return d3.format("~g")(value / 1000000) + "M" + yUnit;
        } else if (value >= 1000) {
            return d3.format("~g")(value / 1000) + "K" + yUnit;
        } else {
            return d3.format("~g")(value) + yUnit;
        }
    }
