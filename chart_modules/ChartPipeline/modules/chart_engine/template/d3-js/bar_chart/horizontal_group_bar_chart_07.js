...
function makeChart(containerSelector, data) {
    ...
    const formattedMaxValueWithUnit = `${formatValue(maxPossibleValue)}${valueUnit}`;
    const tempValueText = tempSvg.append("text")
        .style("font-family", typography.annotation.font_family)
        .style("font-size", typography.annotation.font_size)
        .style("font-weight", typography.annotation.font_weight)
        .text(formattedMaxValueWithUnit);
    ...
    let currentX = 50;
    let currentY = 0;
    const maxLegendWidth = width - 100; // 使用图表内部宽度作为限制
    ...
    groups.forEach((group, i) => {
        ...
        // 更新下一个图例项的起始 X 坐标
        currentX += itemWidth + legendPadding; // 加上图例项之间的间距
    });
