...
// Proportional Area Chart (Square) - 使用D3.js
function makeChart(containerSelector, data) {
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let colIdx = 0; colIdx < itemsInThisRow; colIdx++) {
            ...
            // ---------- 10. 绘制正方形或图标 ----------
            const maxPossibleSide = radius * 2; // 'radius' is half the max dimension of the shape's bounding box
            // 确保 d[valueField] 是数值，避免 Math.sqrt 对非数值进行操作
            const numericValue = parseFloat(d[valueField]);
            const proportionalSide = (maxValue > 0 && !isNaN(numericValue)) ? maxPossibleSide * Math.sqrt(numericValue / maxValue) : 0;
            
            let displaySide = proportionalSide;
            if (proportionalSide > 0 && proportionalSide < 10) { // 最小边长为10
                displaySide = 10;
            }
            // 确保 displaySide 不会是 NaN 或负数
            if (isNaN(displaySide) || displaySide < 0) {
                displaySide = 0;
            }
            ...
            if (displaySide > 0) { // 仅当边长为正时绘制
                ...
                if (iconUrl) {
                    ...
                    // 图标尺寸设为正方形的2倍，确保能完全填满正方形
                    const imageSize = displaySide * 2;
                    ...
                    chartGroup.append("image")
                        ...
                        .attr("preserveAspectRatio", "xMidYMid slice") // 使用slice允许裁剪但保持比例
                        .attr("clip-path", `url(#${clipId})`) // 应用裁剪路径
                        .style("filter", variables.has_shadow ? "url(#shadow)" : "none");

                    // 如果需要描边，在图标外部绘制一个透明的矩形作为边框
                    ...
                    chartGroup.append("rect")
                        ...
                        .attr("stroke-width", variables.has_stroke ? 1 : 0)
                        .style("filter", variables.has_shadow ? "url(#shadow)" : "none")
                        .style("opacity", 0.85); // 保留原彩色矩形的透明度
                }
            }
            
            // ---------- 11. 添加标签 ----------
            chartGroup.append("text") // Dimension Label
                .attr("x", 0)
            ...
            chartGroup.append("text") // 数值标签
                .attr("x", 0)
                .attr("y", (displaySide / 2) + 5) // 紧贴正方形下方 (Y是正方形底边 + 5px间距)
                .attr("text-anchor", "middle")
                .attr("dominant-baseline", "hanging") // 文本的顶部锚点在指定的y坐标
                .style("font-family", typography.annotation.font_family)
                .style("font-size", adjustedValueFontSize)
                .style("font-weight", typography.annotation.font_weight)
                .style("fill", colors.text_color || "#333333") // 统一使用标准文本颜色
