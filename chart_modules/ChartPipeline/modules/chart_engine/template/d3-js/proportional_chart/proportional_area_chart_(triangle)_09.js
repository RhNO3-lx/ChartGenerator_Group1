    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let colIdx = 0; colIdx < itemsInThisRow; colIdx++) {
            ...
            if (displaySide > 0) { // 仅当边长为正时绘制
                ...
                const inCircleRadius = displaySide * Math.sqrt(3) / 6; // 三角形内接圆半径
                const inCircleCenterY = 0; // 内接圆圆心Y坐标 (对于当前trianglePath定义，其重心/内切圆心在0,0)
                ...
                defs.append("clipPath")
                    ...
                    .append("circle")
                    .attr("cx", 0)
                    .attr("cy", inCircleCenterY) // 内接圆圆心Y坐标
                    .attr("r", inCircleRadius); // 内接圆半径
                ...
                if (iconUrl) {
                    ...
                    // 计算图标尺寸，确保能覆盖内接圆
                    const boxSize = inCircleRadius * 2.5; // 比内接圆大，确保完全覆盖
                    
                    // 绘制图标并应用内接圆裁剪
                    chartGroup.append("image")
                        .attr("x", -boxSize / 2)
                        .attr("y", -boxSize / 2) // 居中于内接圆 (因为inCircleCenterY现在是0)
                        .attr("width", boxSize)
                        .attr("height", boxSize)
                        .attr("xlink:href", iconUrl)
                        .attr("preserveAspectRatio", "xMidYMid slice") // 使用slice允许裁剪但保持比例
                        .attr("clip-path", `url(#${clipId})`) // 应用内接圆裁剪路径
                        .style("filter", variables.has_shadow ? "url(#shadow)" : "none");

                    // 如果需要描边，绘制一个无填充的三角形边框
