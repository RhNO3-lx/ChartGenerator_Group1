...
function makeChart(containerSelector, dataJSON) {
    ...
    // 调整三角形间距，为标签预留足够空间
    const horizontalPadding = 35 + iconSize; // 水平间距增加32px，避免图标重叠
    ...
    nodeGroups.each(function(d) {
        ...
        if (isLargeTriangle) {
            ...
            if (catWidth > maxLabelWidth) {
                ...
                // 添加标签背景
                const bgRect = gNode.append("rect")
                    .attr("x", -maxLabelWidth/2 - 5) // 恢复原始位置，仅包含标签文本
                    .attr("y", topLabelY - 2)
                    .attr("width", maxLabelWidth + 10) // 恢复原始宽度，仅包含标签文本
                ...
                if (d.icon) {
                    ...
                    gNode.append("image")
                        ...
                        .attr("x", -maxLabelWidth/2 - 5 - iconSize - 10) // 图标位于文本框左侧，额外向左移动10px
                        .attr("y", topLabelY + (totalHeight - iconSize)/2) // 垂直居中
                        .attr("width", iconSize)
                ...
                // 单行显示
                const bgWidth = catWidth + 10;
                
                gNode.append("rect")
                    .attr("x", -catWidth/2 - 5) // 恢复原始位置，仅包含标签文本
                    .attr("y", topLabelY - 2)
                    .attr("width", catWidth + 10) // 恢复原始宽度，仅包含标签文本
                ...
                if (d.icon) {
                    ...
                    gNode.append("image")
                        .attr("xlink:href", d.icon)
                        .attr("x", -catWidth/2 - 5 - iconSize - 10) // 图标位于文本框左侧，额外向左移动10px
                        .attr("y", topLabelY - 2 + (catFontSize + 4 - iconSize)/2) // 垂直居中
                        .attr("width", iconSize)
                        .attr("height", iconSize)
                        .attr("preserveAspectRatio", "xMidYMid meet");
                }
            ...
            if (needWrap) {
                ...
                // 添加背景
                gNode.append("rect")
                    .attr("x", -maxAllowedWidth/2 - 5) // 恢复原始位置，仅包含标签文本
                    .attr("y", topLabelY - 2)
                    .attr("width", maxAllowedWidth + 10) // 恢复原始宽度，仅包含标签文本
                ...
                if (d.icon) {
                    ...
                    gNode.append("image")
                        .attr("xlink:href", d.icon)
                        .attr("x", -maxAllowedWidth/2 - 5 - iconSize - 10) // 图标位于文本框左侧，额外向左移动10px
                        .attr("y", topLabelY + (totalHeight - iconSize)/2) // 垂直居中
                        .attr("width", iconSize)
                        .attr("height", iconSize)
                        .attr("preserveAspectRatio", "xMidYMid meet");
                ...
                // 添加背景
                gNode.append("rect")
                    .attr("x", -newMaxWidth/2 - 5) // 恢复原始位置，仅包含标签文本
                    .attr("y", topLabelY - 2)
                    .attr("width", newMaxWidth + 10) // 恢复原始宽度，仅包含标签文本
                    .attr("height", totalHeight)
                    .attr("rx", 3)
                    .attr("ry", 3)
                    .attr("fill", "white")
                ...
                if (d.icon) {
                    ...
                    gNode.append("image")
                        .attr("xlink:href", d.icon)
                        .attr("x", -newMaxWidth/2 - 5 - iconSize - 10) // 图标位于文本框左侧，额外向左移动10px
                        .attr("y", topLabelY + (totalHeight - iconSize)/2) // 垂直居中
                        .attr("width", iconSize)
                        .attr("height", iconSize)
                        .attr("preserveAspectRatio", "xMidYMid meet");
