...
function makeChart(containerSelector, dataJSON) {
    ...
    if (nodes.length > 1) {
        ...
        for (let i = 1; i < nodes.length; i++) {
            ...
            // 不要让节点太靠近顶部，避免与图例重叠
            let posX = W/2 + current_radius * Math.cos(angle);
            let posY = H/2 + current_radius * Math.sin(angle) + 20; // 整体下移
            
            // 如果节点靠近顶部，额外下移
            if (posY - nodes[i].radius < 0) {
                posY += nodes[i].radius + 10;
            }
