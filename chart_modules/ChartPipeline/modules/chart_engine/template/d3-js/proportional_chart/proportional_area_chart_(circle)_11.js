    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < itemsInThisRow; col++) {
            ...
            chartGroup.append("text")
                .attr("x", 0)
                .attr("y", -radius - 10) // 恢复: 放在圆上方 (使用单元格最大半径 radius)
            ...
            // 条件判断：标签宽度 < 圆直径*0.9, 标签高度 < 圆直径*0.9, 且圆半径 > 标签高度*0.7
            const canFitInside = labelBBox.width < displayRadius * 2 * 0.9 && 
                                 labelBBox.height < displayRadius * 2 * 0.9 && 
                                 displayRadius > labelBBox.height * 0.7;

            if (canFitInside && displayRadius > 5) { // 使用 displayRadius 判断，并且最小半径为5的圆仍然可以考虑内部文字
                valueLabel
                    .attr("x", 0)
                    .attr("y", 0) 
                    .attr("text-anchor", "middle")
                    .attr("dominant-baseline", "central") 
                    .style("fill", "#FFFFFF"); // 内部文本使用白色
            } else {
                valueLabel
                    .attr("x", 0)
                    .attr("y", displayRadius + 2) // 圆形下方边缘 +2px 间距
                    .attr("text-anchor", "middle")
