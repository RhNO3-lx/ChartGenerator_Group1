...
function makeChart(containerSelector, dataJSON) {
    ...
    const valueFontSizeBase = parseFloat(dataJSON.typography?.annotation?.font_size || '10'); // 数值标签基础字号
    ...
    const categoryFontSizeBase = parseFloat(dataJSON.typography?.label?.font_size || '10'); // 维度标签基础字号
    ...
    nodeG.each(function(d) {
        ...
        while (currentFontSize > minAcceptableFontSize) {
            ...
            if (catText && !categoryFits && needsWrapping && currentFontSize >= minCatFontSize) {
                 ...
                 if (false && words.length <= 1) { // 按字符模拟换行
