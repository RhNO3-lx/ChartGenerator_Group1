...
function makeChart(containerSelector, data) {
    ...
    chartData.forEach(d => {
        ...
        // 绘制条形
        // 使用半圆形状：正值条形右侧半圆，负值条形左侧半圆
        const y = yScale(dimension);
        const height = barHeight;
        const radius = height / 2; // 半圆半径等于条形高度的一半
        
        if (value >= 0) {
            // 正值条形：右侧使用半圆
            const path = [
                `M ${barX},${y}`, // 左上角起点
                `L ${barX + barWidth - radius},${y}`, // 到右侧半圆起点
                `A ${radius},${radius} 0 0,1 ${barX + barWidth - radius},${y + height}`, // 右侧半圆
            ...
            // 负值条形：左侧使用半圆
            const path = [
                `M ${barX + radius},${y}`, // 左侧半圆起点
                `A ${radius},${radius} 0 0,0 ${barX + radius},${y + height}`, // 左侧半圆
