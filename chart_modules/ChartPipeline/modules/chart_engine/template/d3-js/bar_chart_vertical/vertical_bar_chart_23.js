...
// 使用D3.js创建简单的垂直条形图   plain chart#3 顶端三角形
function makeChart(containerSelector, data) {
    ...
    chart.selectAll(".bar")
        ...
        .attr("d", d => {
            ...
            const height = Math.max(0, innerHeight - yScale(d.y));
            
            // 如果高度为0，返回空路径
            if (height <= 0) return "";
            
            // 三角形的高度，设置为条形图宽度，但最小10px，最大30px
            const triangleHeight = Math.min(30, Math.max(10, width));
            
            // 确保三角形高度不超过总高度
            const actualTriangleHeight = Math.min(triangleHeight, height);
            
            // 矩形部分的高度
            const rectHeight = height - actualTriangleHeight;
            ...
            // 到左上角（矩形顶部）
            path += ` L ${x} ${y + actualTriangleHeight}`;
            
            // 到三角形左侧点
            path += ` L ${x + width * 0} ${y + actualTriangleHeight}`;
            
            // 到三角形顶点
            path += ` L ${x + width / 2} ${y}`;
            
            // 到三角形右侧点
            path += ` L ${x + width * 1} ${y + actualTriangleHeight}`;
            
            // 到右上角（矩形顶部）
            path += ` L ${x + width} ${y + actualTriangleHeight}`;
            
            // 到右下角
            path += ` L ${x + width} ${innerHeight}`;
