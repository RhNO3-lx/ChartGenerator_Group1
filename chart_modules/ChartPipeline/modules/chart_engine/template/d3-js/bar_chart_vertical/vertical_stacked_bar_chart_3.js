...
function makeChart(containerSelector, data) {
    ...
    layers.each(function(layerData, layerIndex) {
        ...
        const baseColor = colorScale(layerData.key);
        
        // 计算颜色变化
        const frontColor = baseColor;
        const topColor = d3.color(baseColor).brighter(0.8);
        const sideColor = d3.color(baseColor).darker(0.6);
        
        layerData.forEach((d, i) => {
            const bw = xScale.bandwidth();
            const x0 = xScale(d.data.period);
            const y0 = yScale(d[0]);  // 底部位置
            const y1 = yScale(d[1]);  // 顶部位置
            const height = y0 - y1;
            ...
            layer.append("polygon")
                .attr("points", [
                    [x0 + bw, y1],           // 右上
                    [x0 + bw + dx, y1 - dy], // 右上+偏移
                    [x0 + bw + dx, y0 - dy], // 右下+偏移  
                    [x0 + bw, y0]            // 右下
                ].join(" "))
                .attr("fill", sideColor)
                .style("stroke", variables.has_stroke ? "#ffffff" : "none")
                .style("stroke-width", variables.has_stroke ? 0.5 : 0);
            
            // ===== 顶面 polygon（只有最顶层才有顶面）=====
            // 检查是否是最顶层（在该x位置的最后一个非零高度的层）
            const isTopLayer = layerIndex === stackedData.length - 1 || 
                              !stackedData.slice(layerIndex + 1).some(nextLayer => {
    ...
    layers.each(function(layerData) {
        ...
        layerData.forEach(d => {
            ...
            if (height > 16 && value > 0) {
                layer.append("text")
                    .attr("x", x0 + bw/2 + dx/3)  // 稍微向右偏移以适应3D效果
                    .attr("y", y1 + height/2 - dy/3)  // 稍微向上偏移
