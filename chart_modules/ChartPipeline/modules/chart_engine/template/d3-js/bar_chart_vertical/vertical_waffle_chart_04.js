...
function makeChart(containerSelector, data) {
    ...
    // 计算标题所需高度
    const titleHeight = 100; // 增加高度以适应图标
    ...
    const categoryInfo = chartData.map((d, i) => {
        ...
        return {
            ...
            icon: images.field && images.field[category] ? images.field[category] : null
        };
    });
    ...
    categoryInfo.forEach((info, index) => {
        ...
        if (info.icon) {
            ...
            // 添加图标图像
            grid.append("image")
                .attr("x", gridWidth / 2 - iconSize / 2)
                .attr("y", -titleHeight / 2 - 20 - iconSize / 2)
                .attr("width", iconSize)
                .attr("height", iconSize)
                .attr("xlink:href", info.icon)
                .attr("preserveAspectRatio", "xMidYMid meet");
                
            // 添加百分比文本到图标旁边
            grid.append("text")
                .attr("x", gridWidth / 2 + iconSize / 2 + 15)
                .attr("y", -titleHeight / 2 - 20)
