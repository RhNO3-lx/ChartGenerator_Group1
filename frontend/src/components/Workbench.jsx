    ...
    try {
      const response = await axios.post('/upload', formData, {
        ...
        onUploadProgress: (progressEvent) => {
          ...
          setUploadStatus(`上传进度: ${progress}%`);
        },
      });

      if (response.data.status === 'success') {
        setUploadStatus('上传成功，正在处理数据...');
        
        // 获取处理结果
        const combinedData = response.data;
        console.log('上传的文件已处理完成', combinedData);
        
        // 可以在这里将数据存储到全局状态或缓存中
        // 例如，存储到localStorage或全局状态管理器中
        localStorage.setItem('uploadedStudentData', JSON.stringify(combinedData));
        
        setUploadStatus('数据处理完成！');
        setTimeout(() => {
      ...
      <div className="sidebar">
        ...
        {sidebarView === 'config' ? (
          <>
            ...
            {/* 文件上传部分 */}
            <div className="config-section">
              <div className="section-title">教学数据上传</div>
